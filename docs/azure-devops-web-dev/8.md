...
| 代码和应用编排 | 应用开发人员/运营 | Azure DevOps 管道 | 最开放、最灵活的活动支持、审批队列、带门控的阶段 |
...
COTS 的 DevOps 依赖于应用对实现 DevOps 的支持。一般来说，实现 COTS 的端到端 DevOps 是完全不可能的。目前，大多数 COTS 开发和部署涉及很少的手动任务，DevOps 工具不会处理这些任务。一些主要的 COTS 参与者，如 SAP 和 Saleforce，改变了定制实施和部署的流程，以与 DevOps 保持一致。我们可以实现 DevOps 的某些方面，但仍然涉及一些手动元素来完成完整的发布周期。
