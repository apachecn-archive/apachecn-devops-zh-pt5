...
为了在 Kubernetes 中部署有状态的应用，Kubernetes 集群必须实现一些核心功能。首先，群集需要将存储作为应用的卷调配和连接到 Pod。接下来，根据应用体系结构，群集需要控制对该存储的访问，以实现安全性和应用兼容性。Kubernetes API 公开了几个对象，这些对象支持部署有状态应用所需的供应、配置和访问控制。下面是 Kubernetes 用来支持有状态应用的 API 对象列表:
...
当应用想要连接到集群中的持久卷时，必须创建持久卷声明。持久卷声明是用户对存储的请求。永久卷声明使您可以只向集群请求存储，而不必在 Pod 规范中提供实施细节。这允许应用配置的可移植性，并使您的部署清单在定义 pod 和更高级别的构造时独立于集群，将特定于基础设施的代码从部署代码中分离到专用的存储对象中。
