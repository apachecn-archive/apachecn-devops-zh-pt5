...
自动化部署通常是团队在转向DevOps流程时首先要做的事情之一。这不是没有原因的。自动化部署是一个巨大的进步。自动化的部署过程可以帮助您的团队减少错误，让您的团队专注于重要的事情。本章向您展示了如何使用发布管理来实现自动部署。您将会看到什么是发布管理以及它是如何工作的，并且会看到部署不同应用类型的可能性。
