...
要获得与在 Linux 上运行的容器映像相同的输出，请改为运行以下命令。请注意标签的使用，这是一个专门定义使用什么图像来运行容器的标签。标签的用法将在后面的章节中详细介绍。
...
Docker 映像是一个静态的、*只读的*模板，用于创建应用容器。它是运行应用所需的各种组件的非运行表示。然而，Docker 容器是映像的运行时实例。当您运行`docker run hello-world`命令时， *hello-world* 是 Docker 映像的名称。您可以将 Docker 图像视为蓝图，就像汽车的技术图纸一样。您根据您希望汽车(Docker 容器)的外观和功能创建蓝图(Docker 图像)。蓝图完成后，您可以根据蓝图创建任意多的汽车实例。你可以自由地改变汽车的外观——颜色、配件、内饰等等——而不需要修改蓝图。如果你决定对汽车进行重大修改，你必须从头开始，重写蓝图。
...
您也可以参考 Microsoft Container Registry (MCR)来下载图像:
